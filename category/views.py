...
class DetailCategory(View):
    ...
    def get(self, request, slug):
        ...
        sanpham = SanPham.objects.all().filter(ChuyenMuc=chuyenmuc).filter(TrangThai=True)
        ...
        if(request.GET.get('trang') is not None):
            try:
                ...
                if page > page_count or page <= 0:
                    ...
                    if (request.GET.get('sap_xep') is not None) and request.GET.get('sap_xep').lower() == "giam":
                        sanpham = SanPham.objects.all().filter(ChuyenMuc=chuyenmuc).filter(TrangThai=True).order_by('-GiaBan')[int(start_index):int(end_index)]
                    ...
                    if (request.GET.get('sap_xep') is not None) and request.GET.get('sap_xep').lower() == "moi":
                        sanpham = SanPham.objects.all().filter(ChuyenMuc=chuyenmuc).filter(TrangThai=True).order_by('-id')[int(start_index):int(end_index)]
            ...
            try:
                ...
                if (request.GET.get('sap_xep') is not None) and request.GET.get('sap_xep').lower() == "giam":
                    sanpham = SanPham.objects.all().filter(ChuyenMuc=chuyenmuc).filter(TrangThai=True).order_by('-GiaBan')[0:12]
                if (request.GET.get('sap_xep') is not None) and request.GET.get('sap_xep').lower() == "tang":
                    sanpham = SanPham.objects.all().filter(ChuyenMuc=chuyenmuc).filter(TrangThai=True).order_by('GiaBan')[0:12]
                if (request.GET.get('sap_xep') is not None) and request.GET.get('sap_xep').lower() == "moi":
                    sanpham = SanPham.objects.all().filter(ChuyenMuc=chuyenmuc).filter(TrangThai=True).order_by('-id')[0:12]
                number_page = [i for i in range(1, page_count + 1)]
                data = {
                    "sanpham": sanpham, 
                    "chuyenmuc": chuyenmuc, 
