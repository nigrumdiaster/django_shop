...
class SanPhamAdmin(admin.ModelAdmin):
    ...
    list_per_page = 10
    search_fields = ('TenSanPham', 'MoTaNgan')
    list_filter = ('ChuyenMuc__TenChuyenMuc', 'TrangThai')
    readonly_fields = ('display_hinh_anh',)  # Trường chỉ đọc để hiển thị hình ảnh

    def save_model(self, request, obj, form, change):
        # Check if giakhuyenmai is greater than giaban before saving
        if obj.GiaKhuyenMai is not None and obj.GiaBan is not None and obj.GiaKhuyenMai <= obj.GiaBan:
            self.message_user(request, "Giá khuyến mãi phải lớn hơn giá bán, mặt hàng sẽ không được hiển thị.", level=messages.ERROR)
            obj.TrangThai = False
